...
function(x,method=NULL,threshold=NULL,noise.ctrl=1e-8,...){
	...
	if(is.null(x$com_ev)) d <- pck_hdclassif_dim_choice(x$ev,n,method,threshold,TRUE,noise.ctrl)
	else d <- pck_hdclassif_dim_choice(x$com_ev,n,method,threshold,TRUE,noise.ctrl)
}
