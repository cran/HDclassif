hddc <-
function(data,k=1:10,model='AkiBkQkDk',threshold=0.2,iter.max=60,eps=1e-2,graph=FALSE,algo='EM',d=NULL,init='kmean',show=TRUE,mini.nb=c(5,10),scaling=FALSE,ctrl=1){
	...
	Alg<-c('EM','CEM','SEM')
	Init<-c('random','kmean','mini-em','param')
	model=toupper(model)
	if (!any(Mod==model)) cat("Error : invalid model name\n")
	else if (!is.numeric(ctrl) || ctrl<0) cat("Error : the control variable must be a strictly positive double\n")
	...
	else if (any(model==Mod[7:14]) && length(d)!=0 && d>ncol(data)) cat("Error: d must be strictly inferior to the dimension, \nwhich is in this case ",ncol(data)-1,'\n')
		...
		for (i in sort(k)){
			for (M in model){
				e[[ind]]<-pck_hddc(data,i,M,threshold,d,graph,algo,iter.max,eps,init,mini.nb,ctrl)
				if (length(e[[ind]])==1){
